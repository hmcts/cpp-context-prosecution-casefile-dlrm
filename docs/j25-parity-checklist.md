...
| BC-07 | Liquibase 4→5 removed properties | 📝 | `LiquibasePropertiesKeyInventoryTest` removed on review: it never invoked Liquibase, so it could not fail when Liquibase 5 rejects `liquibase.hub.mode` — it asserted *today's* key set including the stale key, so it would only fail once someone removed the key, i.e. exactly when the real problem gets fixed (an inverted assertion). Fixed directly instead: `liquibase.hub.mode` deleted from `liquibase.properties`. `pcfdlrm.xml`'s changelog is also currently empty (no changesets, no tables) — there is no real viewstore schema here for a migration to affect |
