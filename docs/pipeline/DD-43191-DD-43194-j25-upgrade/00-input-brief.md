...
Sibling story in the other repo: [DD-43192](https://github.com/hmcts/cpp-context-stagingdlrm/blob/main/docs/pipeline/DD-43191-DD-43192-j25-upgrade/00-input-brief.md).
...
ends that state, so it must not start until the parity PR has landed (ADR-005 Method 1, ADR-006
decision 1). Sequence:
...
neither DLRM repo has that dependency, so no decouple PR (ADR-006 decision 3). No `anonymise` module
in this repo either, so ADR-006's open question does not apply.
