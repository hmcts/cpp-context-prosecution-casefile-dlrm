...
> [DD-43192](https://github.com/hmcts/cpp-context-stagingdlrm/blob/main/docs/pipeline/DD-43191-DD-43192-j25-parity/01-requirements.md).
> Method and scope are fixed by [ADR-005](https://github.com/hmcts/cpp-context-stagingdlrm/blob/main/docs/pipeline/adrs/005-j25-parity-test-method-and-bc-scope.md),
...
| Depends on | [ADR-005](https://github.com/hmcts/cpp-context-stagingdlrm/blob/main/docs/pipeline/adrs/005-j25-parity-test-method-and-bc-scope.md) accepted before stage 5. No other blocker — can start immediately |
...
| Sibling story | [DD-43192](https://github.com/hmcts/cpp-context-stagingdlrm/blob/main/docs/pipeline/DD-43191-DD-43192-j25-parity/00-input-brief.md) — same stage in stagingDLRM, independently deliverable |
...
- **FR3 — Scope is closed.** The items in play are exactly ADR-005's Bucket A for this repo: **BC-03,
  BC-07, BC-08, BC-11, BC-12, BC-13, BC-20, BC-21**. `DLRM-01` is stagingDLRM-only (no Function App
  here). Adding an item requires ADR-005 amended. Writing a test for an N/A item — in particular
...
- **FR15 — `docs/j25-parity-checklist.md` is a deliverable.** One row per Bucket A and Bucket B item,
  carrying ADR-005's legend (🟡 · 🟢 · 🔴 · ⬜ · 📝), the J17 run evidence for every 🟢, and an explicit
...
- **These tests will not exist on `main`.** Both stages live on `team/25.104.x`, so the shipping J17
  ...
  BC-03 is already complete on this side (FR11). See ADR-006 decision 1.
...
5. **`WholePayloadMatcher` is shared test-support consumed by DD-43099's suites.** Check
   [ADR-001](https://github.com/hmcts/cpp-context-stagingdlrm/blob/main/docs/pipeline/adrs/001-dlrm-scenario-test-dsl.md) before changing its API rather than
   asserting over it.
