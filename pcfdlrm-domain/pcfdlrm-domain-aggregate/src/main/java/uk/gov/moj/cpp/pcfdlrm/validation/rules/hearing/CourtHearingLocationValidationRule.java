...
public class CourtHearingLocationValidationRule implements ValidationRule<MigratedHearingWithReferenceData, ReferenceDataQueryService> {
    ...
    public ValidationResult validate(final MigratedHearingWithReferenceData migratedHearingWithReferenceData, final ReferenceDataQueryService referenceDataQueryService) {
        ...
        if (isValidOuCode(courtHearingLocation)) {
            final Optional<OrganisationUnitWithCourtroomsReferenceData> optionalOrganisationUnitWithCourtroomReferenceData = referenceDataQueryService.retrieveOrganisationUnitWithCourtrooms(courtHearingLocation);
            if (optionalOrganisationUnitWithCourtroomReferenceData.isPresent()) {
                referenceDataVO.setOrganisationUnitWithCourtroomsReferenceData(optionalOrganisationUnitWithCourtroomReferenceData.get());
                return VALID;
            }
        }
