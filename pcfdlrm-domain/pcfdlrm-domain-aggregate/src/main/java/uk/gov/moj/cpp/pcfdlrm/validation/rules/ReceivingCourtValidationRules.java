...
import static uk.gov.moj.cpp.pcfdlrm.validation.rules.FieldName.RECEIVING_COURT;
...
public class ReceivingCourtValidationRules implements ValidationRule<ProsecutionWithReferenceData, ReferenceDataQueryService> {
    ...
    public ValidationResult validate(ProsecutionWithReferenceData prosecutionWithReferenceData, ReferenceDataQueryService referenceDataQueryService) {
        ...
        final String sendingCourt = prosecutionWithReferenceData.getProsecution().getCaseDetails().getReceivingCourt();

        if (isNull(sendingCourt)) return VALID;

        if (isValidOuCode(sendingCourt) &&
                prosecutionWithReferenceData.getReferenceDataVO().getReceivingCourtOrganisationUnit().isPresent()) {
            return ValidationResult.VALID;
        }

        return newValidationResult(of(newProblem(COURT_LOCATION_OUCODE_INVALID, new ProblemValue(null, RECEIVING_COURT.getValue(), sendingCourt))));
    }
